...
#  endif /*VK_ENABLE_BETA_EXTENSIONS*/
  ...
  template <>
  struct hash<VULKAN_HPP_NAMESPACE::PhysicalDeviceAgentProperties>
  {
    std::size_t operator()( VULKAN_HPP_NAMESPACE::PhysicalDeviceAgentProperties const & physicalDeviceAgentProperties )
      const VULKAN_HPP_NOEXCEPT
    {
      std::size_t seed = 0;
      VULKAN_HPP_HASH_COMBINE( seed, physicalDeviceAgentProperties.sType );
      VULKAN_HPP_HASH_COMBINE( seed, physicalDeviceAgentProperties.pNext );
      for ( size_t i = 0; i < VK_MAX_EXTENSION_NAME_SIZE; ++i )
      {
        VULKAN_HPP_HASH_COMBINE( seed, physicalDeviceAgentProperties.name[i] );
      }
      for ( size_t i = 0; i < VK_MAX_EXTENSION_NAME_SIZE; ++i )
      {
        VULKAN_HPP_HASH_COMBINE( seed, physicalDeviceAgentProperties.version[i] );
      }
      VULKAN_HPP_HASH_COMBINE( seed, physicalDeviceAgentProperties.purposes );
      for ( size_t i = 0; i < VK_MAX_DESCRIPTION_SIZE; ++i )
      {
        VULKAN_HPP_HASH_COMBINE( seed, physicalDeviceAgentProperties.description[i] );
      }
      for ( size_t i = 0; i < VK_MAX_EXTENSION_NAME_SIZE; ++i )
      {
        VULKAN_HPP_HASH_COMBINE( seed, physicalDeviceAgentProperties.layer[i] );
      }
      return seed;
    }
  };
